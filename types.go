...
type PaymentMethodAuthenticationChallenge struct {
	ThreeDs            *ThreeDs
	ThreeDsFingerPrint *ThreeDsFingerPrint
}

func (p *PaymentMethodAuthenticationChallenge) UnmarshalJSON(bytes []byte) error {
	var temp map[string]interface{}
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return err
	}
...
func (p PaymentMethodAuthenticationChallenge) MarshalJSON() ([]byte, error) {
	if p.ThreeDs != nil {
...
type Payment struct {
	...
	AuthenticationChallenge *PaymentMethodAuthenticationChallenge `json:"authentication_challenge"`
